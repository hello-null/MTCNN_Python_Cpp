import torch
import math
import numpy as np
from PIL import Image, ImageDraw
from torch.autograd import Variable
from .get_nets import PNet, RNet, ONet
from .utils import (
    try_gpu,
    nms,
    calibrate_box,
    convert_to_square,
    correct_bboxes,
    get_image_boxes,
    generate_bboxes,
    preprocess,
)




class FaceDetector:
    def __init__(self, device=try_gpu()):
        self.device = device

        # LOAD MODELS
        self.pnet = PNet().to(device)
        self.rnet = RNet().to(device)
        self.onet = ONet().to(device)
        self.onet.eval()

    def detect(
        self,
        image,
        min_face_size=20.0,
        thresholds=[0.6, 0.7, 0.8],
        nms_thresholds=[0.7, 0.7, 0.7],
    ):
        """
        Arguments:
            image: an instance of PIL.Image.
            min_face_size: a float number.
            thresholds: a list of length 3.
            nms_thresholds: a list of length 3.

        Returns:
            two float numpy arrays of shapes [n_boxes, 5] and [n_boxes, 10],
            bounding boxes and facial landmarks.
        """

        # BUILD AN IMAGE PYRAMID
        width, height = image.size
        min_length = min(height, width)

        min_detection_size = 12
        factor = 0.707  # sqrt(0.5)

        # scales for scaling the image
        scales = [] # [0.3, 0.212, 0.15, 0.106, ...]

        # scales the image so that
        # minimum size that we can detect equals to
        # minimum face size that we want to detect
        m = min_detection_size / min_face_size
        min_length *= m

        factor_count = 0
        while min_length > min_detection_size:
            scales.append(m * factor ** factor_count)
            min_length *= factor
            factor_count += 1

        # STAGE 1

        # 用来存放从图像金字塔每一层（不同缩放比例）检测出的候选框。
        bounding_boxes = []

        # 循环不同缩放比例，逐层运行 P-Net
        for s in scales:
            boxes = self.__run_first_stage(image, scale=s, threshold=thresholds[0])
            bounding_boxes.append(boxes)

        # 过滤掉空结果，并将所有层的候选框垂直堆叠
        bounding_boxes = [i for i in bounding_boxes if i is not None]
        bounding_boxes = np.vstack(bounding_boxes)

        # 第一次全局 NMS：跨尺度去重
        keep = nms(bounding_boxes[:, 0:5], nms_thresholds[0])
        bounding_boxes = bounding_boxes[keep]

        # 用 P-Net 输出的偏移量修正候选框坐标
        bounding_boxes = calibrate_box(bounding_boxes[:, 0:5], bounding_boxes[:, 5:])
        # shape [n_boxes, 5]

        # 将所有边界框强制转换为正方形，坐标值四舍五入取整
        bounding_boxes = convert_to_square(bounding_boxes)
        bounding_boxes[:, 0:4] = np.round(bounding_boxes[:, 0:4])

        # STAGE 2

        # 根据候选框从原图裁剪图像块，并调整为 24×24 大小
        img_boxes = get_image_boxes(bounding_boxes, image, size=24)
        with torch.no_grad(): #　将图像块送入 R-Net 进行前向推理
            img_boxes = Variable(torch.FloatTensor(img_boxes).to(self.device))
            output = self.rnet(img_boxes)
            offsets = output[0].cpu().data.numpy()  # shape [n_boxes, 4]
            probs = output[1].cpu().data.numpy()  # shape [n_boxes, 2]

        # 根据人脸概率阈值过滤候选框
        keep = np.where(probs[:, 1] > thresholds[1])[0]
        bounding_boxes = bounding_boxes[keep]
        bounding_boxes[:, 4] = probs[keep, 1].reshape((-1,)) # 将候选框的分数列更新为 R-Net 给出的最新人脸概率（覆盖掉 Stage 1 的旧分数）。
        offsets = offsets[keep] # 

        # 第二次 NMS 去重
        keep = nms(bounding_boxes, nms_thresholds[1])
        bounding_boxes = bounding_boxes[keep]
        bounding_boxes = calibrate_box(bounding_boxes, offsets[keep])
        # 再次强制转换为正方形，并坐标取整
        bounding_boxes = convert_to_square(bounding_boxes)
        bounding_boxes[:, 0:4] = np.round(bounding_boxes[:, 0:4])

        # STAGE 3

        # 根据候选框裁剪 48×48 图像块
        img_boxes = get_image_boxes(bounding_boxes, image, size=48)
        if len(img_boxes) == 0:
            return [], []
        # 将图像块送入 O-Net 进行前向推理
        with torch.no_grad():
            img_boxes = Variable(torch.FloatTensor(img_boxes).to(self.device))
            output = self.onet(img_boxes)
            landmarks = output[0].cpu().data.numpy()  # shape [n_boxes, 10] (x1, x2, x3, x4, x5, y1, y2, y3, y4, y5)
            offsets = output[1].cpu().data.numpy()  # shape [n_boxes, 4]
            probs = output[2].cpu().data.numpy()  # shape [n_boxes, 2]

        # 根据人脸概率阈值过滤候选框
        keep = np.where(probs[:, 1] > thresholds[2])[0]
        bounding_boxes = bounding_boxes[keep]
        bounding_boxes[:, 4] = probs[keep, 1].reshape((-1,))
        offsets = offsets[keep]
        landmarks = landmarks[keep]

        # 将关键点相对坐标转换为原图绝对坐标
        width = bounding_boxes[:, 2] - bounding_boxes[:, 0] + 1.0
        height = bounding_boxes[:, 3] - bounding_boxes[:, 1] + 1.0
        xmin, ymin = bounding_boxes[:, 0], bounding_boxes[:, 1]
        # O-Net 输出的 landmarks 是归一化的相对坐标，数值在 0~1 之间。
        # 例如 landmarks[0, 0] = 0.35 表示第一个关键点（左眼）的 x 坐标位于候选框左边界起 0.35 倍框宽 的位置。
        landmarks[:, 0:5] = (
            np.expand_dims(xmin, 1) + np.expand_dims(width, 1) * landmarks[:, 0:5]
        )
        landmarks[:, 5:10] = (
            np.expand_dims(ymin, 1) + np.expand_dims(height, 1) * landmarks[:, 5:10]
        )

        # 用 O-Net 输出的偏移量修正候选框坐标
        bounding_boxes = calibrate_box(bounding_boxes, offsets)
        # 第三次 NMS 去重（使用 "min" 模式）
        keep = nms(bounding_boxes, nms_thresholds[2], mode="min")
        bounding_boxes = bounding_boxes[keep]
        landmarks = landmarks[keep]

        # bounding_boxes：形状 [F, 5]，每行 (xmin, ymin, xmax, ymax, score)，F 为最终检测到的人脸数。
        # landmarks：形状 [F, 10]，每行 10 个绝对坐标值，依次为：左眼 x, 右眼 x, 鼻子 x, 左嘴角 x, 右嘴角 x, 左眼 y, 右眼 y, 鼻子 y, 左嘴角 y, 右嘴角 y。
        return bounding_boxes, landmarks

    def draw_bboxes(self, image,box_width=3, point_radius=2):
        """Draw bounding boxes and facial landmarks.

        Arguments:
            image: an instance of PIL.Image.

        Returns:
            an instance of PIL.Image.
        """

        bounding_boxes, facial_landmarks = self.detect(image)

        img_copy = image.copy()
        draw = ImageDraw.Draw(img_copy)

        for b in bounding_boxes:
            draw.rectangle([(b[0], b[1]), (b[2], b[3])], outline="white",width=box_width)

        r = point_radius
        for p in facial_landmarks:
            for i in range(5):
                draw.ellipse(
                    [(p[i] - r, p[i + 5] - r), (p[i] + r, p[i + 5] + r)],
                    outline="blue",
                    width=box_width               # 椭圆边框线宽，也可加粗
                )

        return img_copy

    def crop_faces(self, image, size=112):
        """Crop all face images.

        Arguments:
            image: an instance of PIL.Image.
            size: the side length of output images.

        Returns:
            a list of PIL.Image instances
        """

        bounding_boxes, _ = self.detect(image)
        img_list = []

        # convert bboxes to square
        square_bboxes = convert_to_square(bounding_boxes)

        for b in square_bboxes:
            face_img = image.crop((b[0], b[1], b[2], b[3]))
            face_img = face_img.resize((size, size), Image.BILINEAR)
            img_list.append(face_img)
        return img_list

    def __run_first_stage(self, image, scale, threshold):
        """Run P-Net, generate bounding boxes, and do NMS.

        Arguments:
            image: an instance of PIL.Image.
            scale: a float number,
                scale width and height of the image by this number.
            threshold: a float number,
                threshold on the probability of a face when generating
                bounding boxes from predictions of the net.

        Returns:
            a float numpy array of shape [n_boxes, 9],
                bounding boxes with scores and offsets (4 + 1 + 4).
        """

        # scale the image and convert it to a float array
        width, height = image.size
        sw, sh = math.ceil(width * scale), math.ceil(height * scale)
        img = image.resize((sw, sh), Image.BILINEAR)
        img = np.asarray(img, "float32")

        with torch.no_grad():
            img = Variable(torch.FloatTensor(preprocess(img)).to(self.device))
            output = self.pnet(img)
            probs = output[1].cpu().data.numpy()[0, 1, :, :] # 形状 ： [n, m] 每个位置的值表示该位置存在人脸的概率（0-1之间）
            offsets = output[0].cpu().data.numpy() # 形状 ： [1, 4, n, m]
            # probs: probability of a face at each sliding window
            # offsets: transformations to true bounding boxes

        boxes = generate_bboxes(probs, offsets, scale, threshold)
        if len(boxes) == 0:
            return None

        keep = nms(boxes[:, 0:5], overlap_threshold=0.5)
        return boxes[keep]
